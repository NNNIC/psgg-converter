    ...
    function is_contents_1_lc():Void
    {
        ...
        if (!system.Cs2Hx.IsNullOrEmpty(match))
        {
            ...
            if (system.Cs2Hx.StringContains(match, "->@"))
            {
                ...
                if (index >= 0)
                {
                    macro = system.Cs2Hx.Trim_(match.substr(index + 3), [ 36 ]);
                }
            }
            var replacevalue:String = G.get_line_macro_value(macro, m_contents1);
    ...
    function is_macro_lc():Void
    {
        if (m_mw.IsValid() && !m_mw.IsInclude())
        {
            ...
            else
            {
                ...
                {
                    text = Cs2Hx.Format("(error: no value for {0} )", macroname);
                }
                else
                {
